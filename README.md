...
1. 参考jfrascon的思路使用二维数组思路进行多配置文件读取
2. 参考深圳北理莫斯科大学北极熊战队的思路进行SDK内置，实现开箱即用
3. 修改imu发布逻辑，在内部进行外参变换后（重力纠正后）发布
   
Description on the modification:
1. Refer to jfrascon's approach of using a two-dimensional array to read multiple configuration files.
2. Refer to the approach of the Polar Bear Robotics Team at Shenzhen BeiLi Moscow University to integrate the SDK for out-of-the-box functionality. 
3. Modify the imu publishing logic to perform external parameter transformation (gravity correction) internally before publishing



Simon Von Braun's version 1.0.0-2026.3.11

In order to solve the problem of conflicting reads of multi radar configuration files, I tried to adapt Barcelona Bro's open source and learned how the SMBU's PolarBear team integrated livox's SDK into the driver package.
Due to the fact that the original driver directly outputs IMU data, in order to ensure the integration of the feature package, I will simultaneously read the configuration file from IMU and convert it.
Finally,the problem of skewing in pointcloud was solved.
2026.3.11
